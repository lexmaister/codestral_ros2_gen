...
class ROS2CodeParser:
    ...
    code blocks marked with ```python in a model's response. It ensures the code
    ...
    Methods:
        parse(response: str) -> Optional[str]:
            Parse model response: extract and format code only from code blocks
            with ```python markers.
    """
    ...
    def parse(response: str) -> Optional[str]:
        ...
        with ```python at the start and ``` at the end. It then formats the
